...
export default function AnalysesContent() {
  ...
  const handleMarkFailed = async (analysisId: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      toast.success(data.refunded ? 'Başarısız olarak işaretlendi ve krediler iade edildi.' : 'Başarısız olarak işaretlendi.');
    } catch (error: any) {
      toast.error(error.message || 'Analiz durumu güncellenemedi.');
    } finally {
          ...
          <div className="bg-white rounded-xl shadow-sm p-12 text-center">
            ...
            {analyses.length === 0 ? (
              ...
              <>
                ...
                <p className="text-gray-500">Arama veya filtrelerinizi değiştirmeyi deneyin</p>
              </>
            )}
          </div>
        ) : (
                    ...
                    {analysis.status === 'processing' ? (
                      <div className="flex items-center gap-2">
                        ...
                        {isStuckProcessing(analysis) && (
                          ...
                          >
                            {actionLoadingId === analysis.id ? 'Güncelleniyor...' : 'Başarısız olarak işaretle'}
                          </button>
                        )}
                      </div>
